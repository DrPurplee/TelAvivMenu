...
function TA_RestoreOwnSkin()
    ...
    if ok and desc then
        pcall(function()
            sethiddenproperty(LP, "CharacterAppearanceId", LP.UserId)
        end)
...
task.spawn(function()
    ...
    if not skinAutoReapply then return end

    pcall(function()
        sethiddenproperty(LP, "CharacterAppearanceId", skinTargetId)
    end)
    ...
    if applySkinById(skinTargetId, char) then
        ...
        if char2 and skinAutoReapply then
            applySkinById(skinTargetId, char2)
        end
    end
end)
...
-- ══════════════════════════════════════
--  SPINBOT CLEAN STYLE _G.enabled
-- ══════════════════════════════════════
TA_SpinbotActive = false
TA_SpinbotSpeed = 50
TA_SpinbotLoopRunning = false

function TA_CleanSpinSpeed(v)
    local n = tonumber(v)
    ...
    return math.clamp(n, 1, 149)
end

function TA_StartSpinbot()
    TA_SpinbotActive = true

    if TA_SpinbotLoopRunning then return end
    TA_SpinbotLoopRunning = true

    task.spawn(function()
        while TA_SpinbotActive do
            task.wait()
            ...
            if hum and root and root.Parent then
                ...
                local rotspeed = TA_CleanSpinSpeed(TA_SpinbotSpeed)

                root.CFrame = root.CFrame * CFrame.Angles(
                    0,
                    math.rad(rotspeed % 150),
                    0
                )
            end
        end
...
-- ══════════════════════════════════════
--  LOCAL VEHICLE LOCK / UNLOCK — REGISTER FIX
-- ══════════════════════════════════════
vehicleLockActive = false
vehicleLockedModel = nil
vehicleLockCache = {}
vehicleLockConn = nil

function getVehicleParts(vehicle)
    local parts = {}
    if not vehicle or not vehicle.Parent then return parts end
...
function hardStopVehicle(vehicle)
    if not vehicle or not vehicle.Parent then return end

    for _, part in pairs(getVehicleParts(vehicle)) do
        pcall(function()
...
function unlockVehicleLocal()
    if vehicleLockConn then
        vehicleLockConn:Disconnect()
        vehicleLockConn = nil
    end

    if vehicleLockedModel and vehicleLockedModel.Parent then
        for part, props in pairs(vehicleLockCache) do
        ...
        hardStopVehicle(vehicleLockedModel)
    end
...
function lockVehicleLocal(vehicle)
    if not vehicle or not vehicle.Parent then return false end

    unlockVehicleLocal()

    vehicleLockActive = true
    vehicleLockedModel = vehicle
    vehicleLockCache = {}

    hardStopVehicle(vehicle)

    for _, part in pairs(getVehicleParts(vehicle)) do
        vehicleLockCache[part] = {
    ...
    vehicleLockConn = RunService.Heartbeat:Connect(function()
        if not vehicleLockActive or not vehicleLockedModel or not vehicleLockedModel.Parent then
            unlockVehicleLocal()
            return
        end

        for _, part in pairs(getVehicleParts(vehicleLockedModel)) do
            pcall(function()
                part.Anchored = true
                part.AssemblyLinearVelocity = Vector3.zero
                part.AssemblyAngularVelocity = Vector3.zero
...
-- ══════════════════════════════════════
--  VEHICLE RETURN — REGISTER SAFE
-- ══════════════════════════════════════

function TA_GetVehicleHeight()
    local vehicle = detectedVehicle
    ...
    local ok, size = pcall(function()
        return vehicle:GetExtentsSize()
    end)

    if ok and size then
        return math.clamp(size.Y, 3, 8)
    end

    return 5
end

function TA_GetGroundUnderMe()
    local myRoot = getHRP()
    ...
    if getChar() then
        table.insert(exclude, getChar())
    end

    if detectedVehicle then
        table.insert(exclude, detectedVehicle)
    end
    ...
    local result = workspace:Raycast(
        myRoot.Position + Vector3.new(0, 12, 0),
        Vector3.new(0, -140, 0),
        rayParams
    )
    ...
    return myRoot.Position - Vector3.new(0, 3, 0), nil
end

function TA_GetVehicleLookCF(pos)
    local myRoot = getHRP()
...
function TA_GetAboveMeReturnCF()
    local myRoot = getHRP()
    if not myRoot then return nil end

    local groundPos = TA_GetGroundUnderMe()
    if not groundPos then return nil end

    local vehicleHeight = TA_GetVehicleHeight()

    local pos = Vector3.new(
        myRoot.Position.X,
        groundPos.Y + vehicleHeight + 20,
        myRoot.Position.Z
    )

    return TA_GetVehicleLookCF(pos)
end

function TA_TestVehicleSidePosition(offset)
    local myRoot = getHRP()
    if not myRoot then return nil end
    ...
    local vehicleHeight = TA_GetVehicleHeight()
    local worldPos = (myRoot.CFrame * CFrame.new(offset)).Position
    ...
    local result = workspace:Raycast(
        worldPos + Vector3.new(0, 18, 0),
        Vector3.new(0, -120, 0),
        rayParams
    )

    if result and math.abs(result.Position.Y - groundY) <= 4 then
        return Vector3.new(
            worldPos.X,
            result.Position.Y + (vehicleHeight / 2) + 0.35,
            worldPos.Z
        )
    end
...
function TA_GetSafeSideReturnCF()
    local myRoot = getHRP()
    if not myRoot then return nil end
    ...
    for _, offset in ipairs(offsets) do
        chosenPos = TA_TestVehicleSidePosition(offset)
    ...
    if not chosenPos then
        local fallback = (myRoot.CFrame * CFrame.new(0, 0, 8)).Position

        chosenPos = Vector3.new(
            fallback.X,
            groundPos.Y + (vehicleHeight / 2) + 0.35,
            fallback.Z
        )
    end

    return TA_GetVehicleLookCF(chosenPos)
end

function returnVehicleAndLock(shouldLock)
    local aboveCF = TA_GetAboveMeReturnCF()
    local sideCF = TA_GetSafeSideReturnCF()

    if not aboveCF or not sideCF then
        return
    end

    if not tpVehicleTo(aboveCF) then
        return
    end

    task.wait(0.12)

    if tpVehicleTo(sideCF) then
        task.wait(0.35)
...
SecSelf:Slider({
    ...
    Min = 10, Max = 1000, Default = 80, Decimals = 1,
...
for _, badgeName in ipairs(BadgeOrder) do
    table.insert(cosmeticItems, "Badge " .. badgeName)
end
...
SecAimbotSelf:Slider({Name="Taille cercle FOV",Flag="AimbotFOV",Min=10,Max=800,Default=120,Decimals=1,Callback=function(v) aimbotFOV=v; fovCircle.Radius=v; updateFOVCircle() end})
SecAimbotSelf:Slider({Name="Smoothing",Flag="AimbotSmooth",Min=0,Max=90,Default=10,Decimals=1,Callback=function(v) aimbotSmoothing=v/100 end})
...
SecTroll:Toggle({
    ...
    Callback = function(v)
        ...
        if v then
            ...
            TA_StopSpinbot()
            notify("Spinbot", "OFF", 2)
        end
    end,
})

SecTroll:Slider({
    Name = "Spinbot Vitesse",
    Flag = "SpinbotSpeedClean",
    Min = 1,
    Max = 149,
    Default = 50,
    Decimals = 1,
    Callback = function(v)
        TA_SpinbotSpeed = TA_CleanSpinSpeed(v)
    end,
})
...
SecTroll:Button({
    Name = "Map Destroyer",
    Callback = function()
        if TA_LolilolTrack then
            pcall(function()
                TA_LolilolTrack:Stop()
            end)
            TA_LolilolTrack = nil
        end
        ...
        local hum = getHum()
        if not hum then
            notify("Dance", "Humanoid introuvable", 2)
            return
        end
        ...
        if not ok or not track then
            notify("Dance", "Animation impossible", 2)
            return
        end
        ...
        TA_LolilolTrack = track

        notify("Tel-Aviv", "MapDestroyer", 2)
    end,
})
    ...
    Callback=function()
        ...
        if carsAttackActive then
            ...
            if not targetHRP then
                notify("Erreur","Cible introuvable !",2)
                carsAttackActive = false
                return
            end
            ...
            task.spawn(function()
                task.wait(2)
                if not carsAttackActive then return end

                returnVehicleAndLock(true)

                carsAttackActive = false
                notify("Véhicule", "Voiture de retour !", 2)
            end)
        else
            notify("TP Car","Annulé.",2)
        end
    end,
})
...
local SecESPMaster=PageVisual:Section({Name="TrackAll — ",Side=1})

SecSkinVisual = PageVisual:Section({
    Name = "Skin Local",
    Side = 2
})

SecSkinVisual:Toggle({
    Name = "Focus Skin",
    Flag = "FocusSkinToggle",
    Default = true,
    Callback = function(v)
        skinAutoReapply = v
        ...
        local char = getChar()
        if not char then
            notify("Skin", "Personnage introuvable", 2)
            return
        end
        ...
        if v then
            ...
            if applySkinById(skinTargetId, char) then
                notify("Skin", "Focus skin activé", 2)
            else
                notify("Skin", "Impossible d'appliquer Focus skin", 2)
            end
        else
            if TA_RestoreOwnSkin() then
                notify("Skin", "Skin original restauré", 2)
            else
                notify("Skin", "Impossible de restaurer", 2)
            end
        end
    end,
})
...
local PageVehicle=Window:Page({Icon="rbxassetid://138449174107046"})
...
SecCar:Slider({
    Name="Vehicle Speed",Flag="VehicleSpeed",Min=0,Max=100,Default=25,Decimals=1,
    Callback=function(v)
...
SecScripts:Button({
    ...
    Callback = function()
        ...
        notify(
            "SESSION INFO",
            "Vous êtes" .. tostring(playerCount) .. "/" .. tostring(maxPlayers) .. " dans la session",
            5
        )
    end,
})
