...
-- ══════════════════════════════════════
--  VÉHICULE — MÉCANIQUE UNIVERSAL VEHICLE SCRIPT
-- ══════════════════════════════════════
local detectedVehicle = nil

-- Accélération / freinage façon Universal Vehicle Script.
local velocityEnabled = true
local vehicleSpeed = 0.025      -- 25 / 1000
local vehicleBrake = 0.150      -- 150 / 1000
local vehicleStopKey = Enum.KeyCode.P

-- Vehicle Fly façon Universal Vehicle Script.
...
local function detectVehicle()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj.Name:lower():find(username) then
            if obj:FindFirstChildOfClass("VehicleSeat") or obj:FindFirstChildOfClass("Seat") or obj:FindFirstChild("Body") or obj:FindFirstChild("Misc") then
                detectedVehicle = obj
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        if vehicle then
            if detectedVehicle ~= vehicle then
                detectedVehicle = vehicle
                notify("Véhicule", "'" .. vehicle.Name .. "' détecté !", 3)
            end
...
UIS.InputBegan:Connect(function(input, gpe)
    if gpe then return end
    if input.KeyCode == vehicleStopKey then
...
-- ── TROLL OPTION ──────────────────────────────────────────────
local SecTroll=PageOnline:Section({Name="Troll Options",Side=2})
