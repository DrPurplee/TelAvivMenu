...
-- ══════════════════════════════════════
--  VEHICLE REPAIR / AUTO REPAIR
-- ══════════════════════════════════════

TA_AutoRepairEnabled = false
TA_AutoRepairCooldown = 8
TA_LastRepairTry = 0
TA_ServiceManager = nil
TA_GuiService = nil
TA_CarFunction = nil

function TA_GetServiceManager()
    if TA_ServiceManager then
        return TA_ServiceManager
    end

    local ok, result = pcall(function()
        return require(ReplicatedStorage:WaitForChild("SharedServices"):WaitForChild("ServiceManager"))
    end)

    if ok and result then
        TA_ServiceManager = result
        return TA_ServiceManager
    end
...
function TA_GetGuiService()
    if TA_GuiService then
        return TA_GuiService
    end

    local sm = TA_GetServiceManager()
    if not sm then return nil end

    local ok, result = pcall(function()
        return sm:GetService("Gui")
    end)

    if ok and result then
        TA_GuiService = result
        return TA_GuiService
    end
...
function TA_GetCarFunction()
    if TA_CarFunction then
        return TA_CarFunction
    end
    ...
    local ok, result = pcall(function()
        return sm:GetService("Instance"):WaitForChild("Car_Function")
    end)

    if ok and result then
        TA_CarFunction = result
        return TA_CarFunction
    end
...
function TA_GetMyCar()
    return workspace:FindFirstChild(LP.Name .. "'s Car")
end

function TA_GetCarDamage()
    local car = TA_GetMyCar()
    if not car then return nil end

    local damage = car:GetAttribute("Damage")
    if not damage then return nil end

    return tonumber(damage)
end

function TA_CanRepairCar()
    local damage = TA_GetCarDamage()

    if not damage then
        return false, "Aucune voiture / aucun dégât"
    end

    if damage <= 0 then
        return false, "Voiture déjà réparée"
    end

    return true, damage
end

function TA_RepairVehicle(useConfirm)
    local canRepair, info = TA_CanRepairCar()

    if not canRepair then
        notify("Repair", tostring(info), 2)
        return false
    end

    local price = math.round(info * 4)

    if useConfirm then
        local gui = TA_GetGuiService()

        if gui then
            local okConfirm, choice = pcall(function()
                return gui:Disclaimer(
                    "Souhaitez vous réparer votre véhicule pour " .. tostring(price) .. "E ?",
                    { "Réparer", "Annuler" }
                )
            end)

            if okConfirm and choice ~= "Réparer" then
                notify("Repair", "Annulé", 2)
                return false
            end
        end
    end

    local carFunction = TA_GetCarFunction()

    if not carFunction then
        notify("Repair", "Car_Function introuvable", 3)
        return false
    end

    local ok, result = pcall(function()
        return carFunction:InvokeServer("Repair")
    end)

    if not ok then
        notify("Repair", "Erreur InvokeServer", 3)
        return false
    end

    if result then
        notify("Repair", tostring(result), 3)
    else
        notify("Repair", "Repair envoyé", 2)
    end
...
SecCar:Button({
    Name = "Repair véhicule",
    Callback = function()
        TA_RepairVehicle(true)
    end,
})

SecCar:Toggle({
    Name = "Auto Repair véhicule",
    Flag = "AutoRepairVehicle",
    Default = false,
    Callback = function(v)
        TA_AutoRepairEnabled = v
        notify("Auto Repair", v and "ON" or "OFF", 2)

        if v then
            task.spawn(function()
                while TA_AutoRepairEnabled do
                    task.wait(2)

                    local now = tick()
                    if now - TA_LastRepairTry >= TA_AutoRepairCooldown then
                        local canRepair = TA_CanRepairCar()

                        if canRepair then
                            TA_LastRepairTry = now
                            TA_RepairVehicle(false)
                        end
                    end
                end
            end)
